...
import com.lightbend.paradox.tree
import com.lightbend.paradox
...
class TableOfContents(
  ...
  def markdown(location: Location[Page], tocIndex: Int, includeIndexes: List[Int]): Node = {
    markdown(
      location.tree.label.base,
      Some(location),
      nested(location.tree, tocIndex, includeIndexes)
    )
  }
  ...
  def headers(location: Location[Page]): Node = {
    ...
    val tree =
      paradox.tree.Tree.leaf(page.copy(headers = List(paradox.tree.Tree(page.h1, page.headers))))
  ...
  private def headersBelow(
    ...
    case Some(loc) =>
      if (isBelow(
            index,
            includeIndexes,
            loc.tree.label.label.getStartIndex,
            loc.tree.label.includeIndexes
          ))
  ...
  private def isBelow(
    ...
    (tocIncludeIndexes, headerIncludeIndexes) match {
      ...
      case _ =>
        headerIncludeIndexes.headOption.getOrElse(headerIndex) > tocIncludeIndexes.headOption
          .getOrElse(tocIndex)
    }
  }
  ...
  private def subList[A <: Linkable](
    ...
    tree.label match {
      case page: Page =>
        val subHeaders =
          if (headers) items(base + page.path, active, page.headers, depth, expandDepth) else Nil
        val subPages = if (pages) items(base, active, tree.children, depth, expandDepth) else Nil
        optList(subHeaders ::: subPages)
      case header: Header =>
        val subHeaders =
          if (headers) items(base, active, tree.children, depth, expandDepth) else Nil
        optList(subHeaders)
    }
  }
  ...
  private def autoExpand[A <: Linkable](
    ...
    maxExpandDepth flatMap { max =>
      expandDepth.filter(_ < max) orElse                                                                 // currently expanding and still below max
        ...
        (if ((max > 0) && active.exists(_.tree.label == linkable)) Some(0)
         else None) // expand from the active page
    }
  }

  private def link[A <: Linkable](
      base: String,
      linkable: A,
      active: Option[Location[Page]]
  ): Node = {
    ...
    new ClassyLinkNode(
      base + path,
      (classAttributes ++ linkable.group.map("group-" + _)).mkString(" "),
      linkable.label
    )
  }
