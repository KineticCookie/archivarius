...
object Index {
  ...
  def headers(root: RootNode): Forest[Ref] = {
    Tree.hierarchy(headerRefs(root, group = None, includeIndexes = Nil))(
      Ordering[Int].on[Ref](_.level)
    )
  }
  ...
  private def headerRefs(
    ...
    root.getChildren.asScala.toList.flatMap {
      ...
      case node: DirectiveNode if node.format == DirectiveNode.Format.ContainerBlock =>
        ...
        val newGroup = node.attributes
          .classes()
          .asScala
          .find(_.startsWith("group-"))
          .map(_.substring("group-".size))
  ...
  private def linkRef(node: Node, level: Int): Option[Ref] = {
    node match {
      case link: ExpLinkNode =>
        Some(Ref(level, link.url, link.getChildren.get(0), group = None, Nil))
      case other =>
