    ...
    if (!StringUtils.isEmpty(node.getType)) {
      node match {
        case vgn: VerbatimGroupNode =>
          printPreAttributes(printer, vgn.getGroup, vgn.getClasses.asScala)
      ...
      case vgn: VerbatimGroupNode =>
        vgn.getSourceUrl.ifPresent(new Consumer[String] {
          override def accept(sourceUrl: String): Unit =
            printer.print(
              s"""<a class="icon go-to-source" href="$sourceUrl" target="_blank" title="Go to snippet source"></a>"""
            )
        })
      case _ =>
    }
...
object PrettifyVerbatimSerializer extends StyledVerbatimSerializer {
  override def printPreAttributes(
      printer: Printer,
      nodeGroup: String,
