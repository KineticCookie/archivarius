...
case class IncludeNode(included: RootNode, includedFrom: File, includedFromPath: String)
  ...
  override def visit(node: Node, visitor: Visitor, printer: Printer): Boolean = node match {
    case include @ IncludeNode(included, includedFrom, includedFromPath) =>
      ...
      val newLocation = Location(
        Tree.leaf(
          Page.included(includedFrom, includedFromPath, context.location.tree.label, included)
        ),
        context.location.lefts,
        context.location.rights,
        context.location.parents
      )
      printer.print(
