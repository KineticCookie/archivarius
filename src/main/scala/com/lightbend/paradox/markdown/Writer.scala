...
import com.lightbend.paradox.markdown.directives.{Directive, DirectiveSerializer}
import com.lightbend.paradox.markdown.directives.standard.{
  CalloutDirective,
  DependencyDirective,
  ExtRefDirective,
  FiddleDirective,
...
import io.hydrosphere.archivarius.directives.ProtoLinkDirective
...
class Writer(serializer: Writer.Context => ToHtmlSerializer) {
  ...
  def this(
      ...
      serializerPlugins: Seq[Writer.Context => ToHtmlSerializerPlugin] =
        Writer.defaultPlugins(Writer.defaultDirectives)
  ) =
...
object Writer {
  ...
  case class Context(
      ...
      reader: Reader,
      writer: Writer,
      pageMappings: String => String =
        Path.replaceExtension(DefaultSourceSuffix, DefaultTargetSuffix),
      ...
      protoLibrary: RawProtoSchema = RawProtoSchema(Nil, Nil)
  )
  ...
  def defaultPlugins(
      directives: Seq[Context => Directive]
  ): Seq[Context => ToHtmlSerializerPlugin] = Seq(
...
//    context =>
//      ProtoLinkDirective(
//        context.location.tree.label,
//        context.paths,
//        context.pageMappings,
//        context.properties
//      ),
    context =>
      RefDirective(
  ...
  class DefaultLinkRenderer(context: Context) extends LinkRenderer {
    private lazy val imgBase = {
      ...
      val base = context.properties
        .getOrElse("image.base_url", sys.error("Property `image.base_url` is not defined"))
    ...
    override def render(node: ExpLinkNode, text: String): LinkRenderer.Rendering = {
      super.render(
        new ExpLinkNode(
          node.title,
          substituteVarsInString(node.url, context.properties),
