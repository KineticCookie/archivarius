...
import com.lightbend.paradox.markdown.directives.SourceDirective
import com.lightbend.paradox.markdown.directives.standard.{GitHubResolver, IncludeDirective}
...
class ParadoxProcessor(
    reader: markdown.Reader = new markdown.Reader,
    writer: markdown.Writer = new markdown.Writer
) {
  ...
  def process(
    ...
    val navToc = new markdown.TableOfContents(
      pages = true,
    ...
    def render(
      ...
      case Some(loc) =>
        ...
        val writerContext = markdown.Writer.Context(
          loc,
          paths,
          reader,
          writer,
  ...
  case class PageContents(
      ...
      context: markdown.Writer.Context,
      navToc: markdown.TableOfContents,
      pageToc: markdown.TableOfContents
  ) extends template.PageTemplate.Contents {
    ...
    lazy val getHome: template.PageTemplate.Link = link(Some(loc.root))
    lazy val getPrev: template.PageTemplate.Link = link(loc.prev)
    lazy val getSelf: template.PageTemplate.Link = link(Some(loc))
    lazy val getNext: template.PageTemplate.Link = link(loc.next)
    ...
    private def link(location: Option[Location[Page]]): template.PageTemplate.Link =
      PageLink(location, page, writer, context)
    private def githubLink(location: Option[Location[Page]]): template.PageTemplate.Link =
  ...
  case class PageLink(
      ...
      writer: markdown.Writer,
      context: markdown.Writer.Context
  ) extends template.PageTemplate.Link {
  ...
  def parseMarkdown(
    ...
    mappings map {
      case (file, path) =>
        val frontin = markdown.Frontin(file)
  ...
  private def processIncludes(
    ...
    root.getChildren.asScala.foreach {
      case include: DirectiveNode if include.name == "include" =>
        ...
        val frontin     = markdown.Frontin(includeFile)
